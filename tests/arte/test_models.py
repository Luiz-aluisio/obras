...
def test_remover_autor_com_obra(db,autor, obra):
    ...
    assert 'autor não pode ser excluido pois possui obras'in str(excinfo.value)
...
def test_adcionar_obra_sem_data_de_publicacao_ou_data_de_exposicao(obra):
    ...
    assert 'obrigatoria caso a data de publicacao nao seja informada' in str(excinfo.value)
    assert 'obrigatoria caso a data de exposicao não seja informada' in str(excinfo.value)
