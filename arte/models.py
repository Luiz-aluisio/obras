...
class Autor(models.Model):
    ...
    sexo = models.CharField(
        max_length=1, choices=[('F', 'Femino'), ('M', 'Masculino')]
    )
    ...
    def clean(self):
        if self.nacionalidade.code == 'BR' and not self.cpf:
            raise ValidationError(
                {'cpf': _('se nacionalidade brasil cpf obrigatorio')}
            )
        if self.nacionalidade.code != 'BR' and self.cpf:
            raise ValidationError(
                {'cpf': _('cpf somente para nacionalidade brasil')}
            )
        return super().clean()
    ...
    def delete(self, *args, **kwargs):
        if self.obras.exists():
            raise ValidationError(
                _('autor não pode ser excluido pois possui obras')
            )
...
class Obra(models.Model):
    ...
    data_de_exposicao = models.DateField(
        auto_now=False, auto_now_add=False, null=True, blank=True
    )
    autores = models.ManyToManyField(
        Autor, related_name='obras', null=True, blank=True
    )
